...
export default function Events() {
  ...
  const toggle = (dept) => setOpenDept(prev => prev === dept ? null : dept);
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="flex items-center gap-4 mb-10">
          ...
          <span className="text-[10px] font-black text-light-blue uppercase tracking-[0.4em]">All Departments</span>
        </div>

        <div>
          {grouped.map(({ dept, color, events }) => (
            <div key={dept} className="border-t border-slate-200 last:border-b last:border-slate-200">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-base font-black text-navy-blue tracking-tight group-hover:text-atlantic transition-colors">
                    {dept}
                  </span>
                ...
                <span className={`text-slate-400 text-lg transition-transform duration-200 ${openDept === dept ? 'rotate-45' : ''}`}>
                  +
                </span>
              </button>

              {/* Expanded events — original card styles preserved */}
              {openDept === dept && (
                <div className="pb-8">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {events.map((event, i) => (
                      <div
                        ...
                        className="group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 flex flex-col"
                      >
                        {/* Image area — original size and logic */}
                        <div
                          className="relative overflow-hidden flex items-end"
                          style={{ backgroundColor: color, aspectRatio: '389 / 315' }}
                        >
